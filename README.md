...
一个基于 Playwright 的淘宝 / 天猫订单发票辅助脚本，用来半自动完成发票规划、申请、换开与下载。

适合已经有人工登录和人工兜底流程的场景：脚本负责处理重复操作，你保留最终确认和异常接管能力。

## 适用场景

- 已经有稳定的人工登录流程
- 需要批量处理淘宝 / 天猫历史订单发票
- 希望减少重复点击，但不想失去人工兜底能力

## 核心能力

- 按时间范围扫描订单并识别可处理状态
- 自动规划“申请开票 / 换开 / 下载 / 跳过”动作
- 支持企业票自动填写与提交
- 支持进度落盘与断点续跑
...
```bash
npm run execute:all -- --start-date=2025-01-01 --end-date=2025-12-31 --max-pages=60 --close-on-finish
```
